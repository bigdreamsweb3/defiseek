...
export const walletMetricsAgent = ApiClient.define({
  ...
  async run({
    ...
    if (!blockchain) {
      throw new Error('⚠️ No blockchain provided to walletMetricsAgent.');
    }
    ...
    const url = `https://api.unleashnfts.com/api/v2/wallet/metrics?blockchain=${blockchain}&wallet=${wallet}&time_range=all&offset=0&limit=30`;
    ...
    try {
      ...
      if (!walletMetrics) {
        throw new Error(`⚠️ No wallet metrics found for address: ${wallet}`);
      }
      ...
      throw new Error(`walletMetricsAgent failed for address: ${wallet}`);
    }
  },
});
