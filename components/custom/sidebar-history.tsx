...
import subtract from 'dayjs/plugin/relativeTime';

dayjs.extend(isToday);
dayjs.extend(isYesterday);
dayjs.extend(subtract); // or use dayjs().subtract()
...
export function SidebarHistory({ user }: { user: UserLike }) {
  ...
  const groupChatsByDate = (chats: Chat[]): GroupedChats => {
    ...
    return chats.reduce(
      (groups, chat) => {
        const chatDate = dayjs(chat.createdAt); // ✅ wrapped with dayjs
        if (dayjs(chatDate).isToday()) {
          groups.today.push(chat);
        } else if (dayjs(chatDate).isYesterday()) {
          groups.yesterday.push(chat);
        } else if (chatDate.isAfter(oneWeekAgo)) {
          groups.lastWeek.push(chat);
        } else if (chatDate.isAfter(oneMonthAgo)) {
